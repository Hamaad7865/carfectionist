...
--
-- ─── ORDERING, and the line that must not be dropped ───────────────────────
-- 20260909000010 (a revision cannot leave a bill behind) also regenerates
-- revise_quote, to refuse revising a quote whose counter bill is still
-- standing. It runs FIRST, and this file replaces the same function — so the
-- guard is repeated below. Without that one line, applying this migration
-- would silently delete it and re-open the double-billing hole: the function
-- would look correct, every test of THIS change would pass, and nothing would
-- report the loss. The two changes are complementary, not rival — one refuses
-- a revision over a live bill, the other retires the superseded quote — and
-- both belong in the body.
-- ═══════════════════════════════════════════════════════════════════════════
...
begin
  ...
  -- From 20260909000010, and repeated here because this file replaces the same
  -- function: a quote already billed on a bill no job owns is past negotiating.
  -- Dropping this line re-opens the hole INV-0204 came through.
...
-- ─── prove BOTH guards survived ─────────────────────────────────────────────
do $$
declare v_def text;
begin
  select pg_get_functiondef('public.revise_quote(uuid)'::regprocedure) into v_def;
  if position('revision_of' in v_def) = 0 then
    raise exception 'revise_quote does not stamp revision_of';
  end if;
  -- The one that a regeneration silently drops. It has no test of its own here,
  -- because the test that would catch it lives in the OTHER migration's script.
  if position('assert_no_superseded_bill' in v_def) = 0 then
    raise exception 'revise_quote lost the superseded-bill guard from 20260909000010';
  end if;
  if position('vehicle_id' in substring(v_def from position('insert into public.document_lines' in v_def))) = 0 then
    raise exception 'the copied lines no longer carry their car';
  end if;
  raise notice 'revise_quote stamps revision_of, keeps each line''s car, and still refuses a quote with a bill standing';
end $$;
