...
**Sales Journal: earlier bills out of Payments (built, verified, NOT committed).** Ask: the "…of which settled earlier bills" row leaves the PAYMENTS card for its own component below. Screen: new "Settled earlier bills" card directly under Payments (same expandable bill rows with the "billed {date}" chips, comparison support, hidden at zero); Payments total untouched — the money IS received, only the presentation moved. PDF: same move — own "Settled earlier bills" table (per-bill rows + total) instead of the note inside Payments. Data layer untouched. `sales-journal-pdf.test.ts` +1 (own table right after Payments, bill row, total; Payments notes empty); full web suite 49 files / 702 tests green; `tsc` clean.

**Deposit breakdown on the slip (built, verified, NOT committed).** Ask: with a deposit paid, the receipt must show the calculation — what was paid the first time and the second — instead of one grouped "2 CASH" row. Rule: EVERY tender leg stands as its own dated row with its time Shared pure helper `tenderRows()` consumed by both the thermal render and the on-screen preview so paper and screen agree. Files: `Hardware.kt` (helper + thermal), `CounterScreen.kt` (preview). `ReceiptTextTest` updated (split legs dated; deposit legs dated with time); suite green; APK on `pos_tablet`.

# Release 0.1.678 � owner's APK (2026-09-12, published, NOT committed � release artifact only)

Signed release (Carfectionist key, versionCode 678) published to the app-releases bucket: carfectionist-678.apk + latest.json. Previous manifest was v666 (9 Sep). Tablets on older builds get the update offer on launch (refresh); same-key install, no uninstall needed. Verified: public manifest fetch, APK HEAD 16602193 bytes, emulator (676) restarted to pick up the offer. Contents since v666: deposit end-to-end (intent, job-card button, ticket + slip), earlier-bills journal section, bughunt criticals + majors (commits 2ea4c63, bd5f06f, 064b5a2).
**Cashmag period picker + receipt/app perf (built, verified, committing now).** Ask: all From/To pickers follow the Cashmag screenshots + a Validate button after selection; receipt view and the whole app slow. New shared PeriodDatePicker (Period date field D/M/YYYY - D/M/YYYY, year <> month-grid <> day-grid popup, staged, Validate commits one navigation) wired into DateRangeFilter (reports/sales/activity/POS), sales-journal PeriodPicker, dashboard SalesPeriodControls (93-day rule as inline popup error). Perf: receipt popup streams via TicketPopupLoader Suspense boundary (list no longer waits 8+ receipt queries); listDocuments 3 archive reads parallel; receipt skips device-registry read without a till session; notifications poll 90s to 5min + hidden-tab skip. PeriodDatePicker.test +24, dashboard tests updated to the new UI; full suite 50 files / 726 green; tsc clean; webpack build green. No DB writes, number series untouched.
**Multi-car quotes in web builder + tablet picker join (built, verified, committing now).** Ask: intake ticks several cars but quotes could not; receipt must group services per car; products print as their own lines with car kept. Found: DB/RPCs, receipt/A4 grouping and tablet builder sections already existed; missing were the web builder UI and a tablet picker bug (tapping a second existing car REPLACED the first). Web: Cars tick-list + inline New car in DocumentBuilder, active-car tabs, grouped editor with plate headings + per-car ex-VAT subtotals, per-line car select, new service/product lines land in the open section attributed (No-car tab = unheaded counter block); preview feeds vehicle per line; edit page re-derives sections. Tablet: picker tap joins via addQuoteCar when cars exist, on-quote marker, No-car row hidden past one car. Tests: state cars/grouping +13, toDocumentProps car mapping +4; full web suite 50 files / 740 green; tsc clean; webpack build green; tablet quote suite 81/81 green. Tablet one-liner rides the next APK. No DB writes.
**All-jobs 500 (found + fixed, shipping now).** Jobs > All jobs died with 'column document_jobs.id does not exist' (caught in the production worker tail): listJobs pages the document_jobs junction through fetchAllRows, whose default ordering is by id � a column that junction table never had (PK is document_id, job_id). Broken since the Sep-5 multi-car change; surfaced now. Fix: order by the PK pair. Audited every other fetchAllRows caller against information_schema � all remaining tables have id (stock_on_hand already passes its compound key). Suite 740 green; tsc/eslint clean.
**Revise updates the quote itself (built, verified, NOT committed).** Ask: revising a quote must update the current quote, not fork a new draft and leave the old one behind. Owner chose full in-place. DB 20260910000110 (pushed live): revise_quote reopens the SAME row - draft comes straight back (no more draft-forks), issued/accepted keeps number and job with an audit row; declined/expired/void, billed and delivered-job quotes still refuse. save_draft (live body, splices kept) persists edits onto issued/accepted quotes under the same guards. Web: ReviseButton title + same-id edit push, detail page hides Revise on declined/expired/void, edit page banner on non-draft. Tablet: QuoteState.amending unlocks the builder in place (quoteEditable, new ReviseInPlaceTest 5/5), revise flips to amend on the same quote, save/accept/bill/send persist first when amending. Verify: probe scripts/_verify-quote-revise-in-place.mjs rolled back (number series untouched - table counter); web suite 50 files / 743 green; tablet quote suite 86/86 green; debug APK built. Old forks keep their links; nothing backfilled.
**Revise hardening sweep (built, verified, NOT committed).** Adversarial pass over in-place revise found 12 holes before customers met them; fixed in 20260910000120 (pushed live) + clients. Critical: web builder was still fully read-only on issued quotes - amend mode added (BuilderState.amending, delete/issue/type/customer frozen, convert + ask-owner work). Stale-bill trap: draft invoices now fence quote edits. Regression guard: fully-credited bills retire again. Holes closed server-side: discount allowance asserted on non-draft saves, doc_type/customer/job frozen once issued, amend saves audited (quote_edited_in_place), revise side-effect-free until first save. Tablet: amending clears on accept/bill success. Verify: probe 24/24 rolled back; web 743 green; tablet quote suite 86/86; fresh debug APK built. Side benefit: owner discount overrides now survive a revise (same doc id - a fork orphaned them).
**Revise keeps the agreement (built, verified, NOT committed).** Owner refinement: a revise is a correction to the same agreement - no re-sign ceremony. 20260910000130 (pushed live) drops un-signing from amend saves: status, signature and booking (time + deposit) all carry, only the figures move. Amend audit records old+new totals. Clients: tablet save keeps signed, web banner reworded. Verify: probe 25/25 rolled back; web 743 green; tablet quote suite 86/86; fresh debug APK on emulator-5554.
**Update-without-ceremony for in-place revises (built, verified, NOT committed).** Owner report with screenshots: after revise + add line, Accept-to-update-job dragged them through the signature pad instead of confirming Quote updated. Cause: footer only offered quiet Update to old forks (revisionOf); same-row amends always got the ceremony. Fix client-only (server already idempotent: same job back, signature kept): footer shows Update when revisionOf set OR (amending + signed + no live bill), new tested showQuoteUpdate rule; success reads Quote updated on the button; car picker Skip renamed OK once a customer is picked. Verify: tablet quote suite 91/91 (new ShowQuoteUpdateTest 5/5); fresh debug APK installed + launched on emulator-5554. No DB change. Follow-up noted, not done: single-car job notes keep the old first-line title after an in-place Update; old-fork Update-and-rebill dialog over-promises where the server refuses (pre-existing).
# Release 0.1.692 - owner's APK (2026-09-23, published - release artifact only)

Signed release (Carfectionist key, versionCode 692) published to the app-releases bucket: carfectionist-692.apk + latest.json (16.6 MB). Previous manifest was v678 (12 Sep). Tablets on older builds get the update offer on launch (refresh); same-key install, no uninstall needed. Verified: public manifest fetch (code 692, notes present), APK URL serves. Contents since v678: revise-in-place quotes (same row/number, agreement kept, Update without ceremony, car picker OK) + hardening/keep-agreement migrations 110/120/130 (all pushed live before this build). Web part shipped in the same window: commit 9dc2669 deployed via the main workflow (tests + webpack build + OpenNext bundle + deploy + smoke test all green).
