...
import { shelfCents, netFromGrossCents, formatMUR, computeTotals, computeLineTotals, parseMoneyInput } from "@/lib/money";
...
export function CounterSale({
  ...
  const totals = useMemo(
    () => computeTotals(
      cart.map((l) => ({ qty: l.qty, unitCents: l.product.priceCents, discountPct: l.discountPct, discountKind: l.discountKind, discountAmountCents: l.discountAmountCents, vatRatePct: l.product.vatRate, priceInclusive: l.product.priceIncludesVat })),
              ...
              >
                ...
                {unpriced ? (
                  ...
                  <span className="num mt-auto text-[13px] font-extrabold text-ink">
                    {formatMUR(shelfCents(p.priceCents, p.vatRate, pricesInclVat, p.priceIncludesVat))}
                  </span>
                )}
              </button>
              );
      ...
      <div className="flex min-w-0 flex-col rounded-[15px] border border-line bg-card">
        ...
        <div className="min-h-[120px] flex-1 overflow-y-auto px-4 py-2">
          {cart.length === 0 ? (
            ...
            cart.map((l) => {
              const lt = computeLineTotals({ qty: l.qty, unitCents: l.product.priceCents, discountPct: l.discountPct, discountKind: l.discountKind, discountAmountCents: l.discountAmountCents, vatRatePct: l.product.vatRate, priceInclusive: l.product.priceIncludesVat });
              ...
              <div key={l.product.id} className="flex items-center gap-2 border-b border-line py-2.5">
                <div className="min-w-0 flex-1">
                  ...
                  {/* The bill states ex-VAT prices (owner decision, 2026-08-14): a flagged product's
                      stored price IS the shelf gross, so its rate is backed out; an unflagged one is
                      stored net already. The tile above keeps the shelf figure — that is the tag. */}
                  <div className="num text-[11px] text-faint">{formatMUR(l.product.priceIncludesVat ? netFromGrossCents(l.product.priceCents, l.product.vatRate) : l.product.priceCents)} each</div>
                </div>
