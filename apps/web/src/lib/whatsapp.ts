...
// ─── ID discovery ────────────────────────────────────────────────────────────
// Meta buries the WhatsApp Account ID and Phone number ID in a console that
// gets renamed every few months. With only the TOKEN we can find them: a system
// user token's granular scopes name the WABAs it was granted, and each WABA
// lists its phone numbers. Two fewer things to hunt for on setup day.
...
export async function discoverIds(): Promise<WaResult<DiscoveredWaba[]>> {
  const e = waEnv();
  if (!e.token) return { ok: false, error: "Set WHATSAPP_TOKEN first — then this can find the rest." };

  // The token describes itself: granular_scopes[].target_ids are the WABAs the
  // System User was assigned.
  const dbg = await graph(`debug_token?input_token=${encodeURIComponent(e.token)}`, { method: "GET" }, e.token);
  if (!dbg.ok) return dbg;

  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const scopes = ((dbg.data as any)?.data?.granular_scopes ?? []) as { scope: string; target_ids?: string[] }[];
  const wabaIds = [
    ...new Set(
      scopes
        .filter((s) => s.scope === "whatsapp_business_messaging" || s.scope === "whatsapp_business_management")
  ...
  if (wabaIds.length === 0) {
    return {
      ok: false,
      error:
        "The token isn't assigned to a WhatsApp account. In Meta: Business settings → Users → System users → your user → Assign assets → add the WhatsApp account with Full control, then regenerate the token.",
    };
  }

  const out: DiscoveredWaba[] = [];
  for (const wabaId of wabaIds) {
    const [meta, nums] = await Promise.all([
      graph(`${wabaId}?fields=name`, { method: "GET" }, e.token),
      graph(`${wabaId}/phone_numbers?fields=id,display_phone_number,verified_name,quality_rating`, { method: "GET" }, e.token),
    ]);
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const rows = (nums.ok ? ((nums.data as any).data ?? []) : []) as any[];
    out.push({
      wabaId,
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      name: meta.ok ? String((meta.data as any).name ?? "WhatsApp account") : "WhatsApp account",
      numbers: rows.map((n) => ({
        phoneNumberId: String(n.id),
        display: String(n.display_phone_number ?? ""),
        verifiedName: String(n.verified_name ?? ""),
        quality: n.quality_rating ? String(n.quality_rating) : undefined,
      })),
    });
  }
